...
"""
import pytest
from app.security_guard import (
...
def test_prompt_injection_detection():
    ...
    for p in malicious_prompts:
        ...
        assert is_suspicious is True, f"Failed to detect injection in: {p}"
        assert len(reason) > 0
    ...
    for sq in safe_queries:
        ...
        assert is_suspicious is False, f"False positive on safe query: {sq}"


def test_assert_safe_query_raises_security_error():
...
def test_tag_breakout_sanitization():
    ...
    assert "</journal_entry_untrusted>" in wrapped[-30:] # only at the real end
